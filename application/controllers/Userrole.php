...
class Userrole extends User_Controller
{
    ...
    public function livejoin()
    {
     ...
     //  print_r($arrayHomework);die;
       
            $this->db->insert('homework_upload', $arrayHomework);
      ...
      // echo     $insert_id;die;
         if (isset($_FILES["fileToUpload"]) && !empty($_FILES['fileToUpload']['name'])) {
            ...
            $file_name =  $post['homework_id'] .'-'. get_loggedin_user_id().'-'.date("d-m-Y").'.' . $fileInfo['extension'];
            move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $uploaddir . $file_name);
        } 
        $this->db->where('id', $insert_id);
        $this->db->update('homework_upload', array('document' => $file_name));
                set_alert('success', translate('information_has_been_updated_successfully'));
               redirect( base_url('userrole/homework'));
