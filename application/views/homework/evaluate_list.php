						...
						if (count($homeworklist)) {
							foreach ($homeworklist as $key => $row) {
								...
								<tr>
									...
									<td><?php $this->db->select('document');
									$this->db->where('homework_id', $this->uri->segment(3));
									$this->db->where('created_by', $row['student_id']);
									$r = $this->db->get('homework_upload')->row_array(); 
									if (empty($r['document'])) { 
										echo '<span style="color: red;">Not Uploaded</span>';
									} else {
									?>
										<a href="<?=base_url('homework/download/' .$r['document'])?>" class="mail-subj"><?=$r['document']?>
									<?php } ?>
										</td>
										<td class="min-w-sm">
